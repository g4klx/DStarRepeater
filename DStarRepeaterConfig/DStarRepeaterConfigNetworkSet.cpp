...
CDStarRepeaterConfigNetworkSet::CDStarRepeaterConfigNetworkSet(wxWindow* parent, int id, const wxString& title, const wxString& gatewayAddress, unsigned int gatewayPort, const wxString& localAddress, unsigned int localPort, const wxString& name) :
wxPanel(parent, id),
m_title(title),
...
m_name(NULL)
{
	...
	wxStaticText* nameLabel = new wxStaticText(this, -1, _("Name"));
	sizer->Add(nameLabel, 0, wxALL | wxALIGN_RIGHT, BORDER_SIZE);

	m_name = new wxTextCtrl(this, -1, name, wxDefaultPosition, wxSize(ADDRESS_WIDTH, -1));
...
CDStarRepeaterConfigNetworkSet::~CDStarRepeaterConfigNetworkSet()
{
}
...
wxString CDStarRepeaterConfigNetworkSet::getGatewayAddress() const
{
	return m_gatewayAddress->GetValue();
}

unsigned int CDStarRepeaterConfigNetworkSet::getGatewayPort() const
{
	unsigned long n;
...
wxString CDStarRepeaterConfigNetworkSet::getLocalAddress() const
{
	return m_localAddress->GetValue();
}

unsigned int CDStarRepeaterConfigNetworkSet::getLocalPort() const
{
	unsigned long n;
...
wxString CDStarRepeaterConfigNetworkSet::getName() const
{
